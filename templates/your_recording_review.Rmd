...
```{r, eval =FALSE}
params <- targets::tar_read(user_params__22727)
```
...
if(treatment_group == "self"){
  ...
  long_delta <- sprintf("%+d",(data_all_lists %>% filter(year == 2025,n_species > 3) %>% nrow() - data_all_lists %>% filter(year == 2024,date < Sys.Date(),n_species>3) %>% nrow()))
  ...
  long_delta <- sprintf("%+d",data_all_lists %>% filter(year == 2025,n_species > 3) %>% nrow() - key_stats$avg_longer_lists)
  unique_delta <- sprintf("%+d",data_all_lists %>% filter(year == 2025) %>% pull(site_name) %>% unique() %>% length() - key_stats$avg_sites)
  
  message1 <- paste0("Compared to the typical **recorder of ",target_group,"**:")
  ...
  long_delta <- data_all_lists %>% filter(year == 2025,n_species > 3) %>% nrow()
...
In 2025, you've been out on **`r data_all_lists %>% filter(year == 2025) %>% nrow()`** visits across **`r data_all_lists %>% filter(year == 2025) %>% pull(site_name) %>% unique() %>% length()`** different 1km grid squares, which we will consider as 'sites'.  Of these visits, **`r data_all_lists %>% filter(year == 2025,n_records ==1 ) %>% nrow()`** were records of single species, **`r data_all_lists %>% filter(year == 2025,n_species %in% c(2,3) ) %>% nrow()`** were shorterlists of 2-3 species and **`r data_all_lists %>% filter(year == 2025,n_species >3 ) %>% nrow()`** were longer lists of 4 or more species. 

From your recording activity you've generated **`r data_all %>% filter(year == 2025) %>% pull(n_records) %>% sum()`** records. **`r n_y1`** of these records were in your most visited site.
    ...
    <td align="center">
      <table cellpadding="0" cellspacing="0" border="4" style="border-collapse: collapse; border: 4px solid ">
        ...
        <tr>
          ...
          <td style="width: 150px; padding: 15px; font-family: Arial, sans-serif; font-size: 14px; text-align: center;"><h3>Longer lists<span style="color: #FC6DAB">*</span> </h3><h2 style="color: #FC6DAB">`r long_delta`</h2></td>
        </tr>
      </table>
    </td>
  </tr>
...
<span style="color: #FC6DAB">\*</span>Recording all species of `r target_group` that you observe on a visit helps build our understanding of the area's biodiversity. This may result in a longer list but this dependant on other factors such as time of year, location and weather. 


```{r, fig.height=7.5}
lollipop_plot_data <- data_all_lists
false_labels <- c("2025" = "This year (2025)","2024" = " Last year (2024)")
...
if(treatment_group=="background"){
  ...
  false_labels <- c(
    "2025" = "Your recording in 2025",
    "2024" = "Other recorders in 2025"
    )
...
if(treatment_group=="self"){
  lollipop_plot_data <- lollipop_plot_data %>% filter(year ==2025) %>% bind_rows(data.frame(list_type = c("Single species","Longer list","Shorter list"),date = as.Date("2020-01-01"),year = "2024",n_species=0))
}
...
ggplot(lollipop_plot_data,aes(x = date,y = n_species,fill = year,colour = year))+
  ...
  annotate("text", x = as.Date("2025-01-05"), y = 2.7, label = "shorterlists",hjust = 0,alpha = 0.5)+
...
lollipop_plot_data %>%
  ...
  theme_minimal(base_size = 20)+
  labs(x= "Date",y = "Number of visits per week")+
  scale_fill_manual(
    values = c(
 ...
 * Record the same place consistently to help understand trends
...
cat(paste0("<a href='https://forms.office.com/Pages/ResponsePage.aspx?id=NlECf_M5LE-jxsuxziGzal97bLz2Q2BBrnNyNFwVq4NURDBXU0k1RFdVREJDVEVJMTlGNFBBRkM2Ty4u&r6cdf8297338f44b2a40063ab6c30a18a=",1:10,"&r2ed61d88722b4e12955003f0cc4cb633=",params$user_email,"'>",1:10,"</a> "))
